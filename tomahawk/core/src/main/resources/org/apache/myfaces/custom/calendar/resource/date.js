...
var global_context;
daysInMonth = function(anyDateInMonth)
{
	var month = anyDateInMonth.month;
	return new Date(anyDateInMonth.year,
		  		  ++month,
		  		  0).getDate();
}

org_apache_myfaces_dateformat_get_context = function() {
	if(global_context == false) {
		return false;
	} else {
		return true;
	}
...
org_apache_myfaces_dateformat_set_context = function(flag) {
	global_context = flag;
}
  ...
  symbols, yearIsWeekYear, context, patternSub, dateStr, dateIndex)
{
  ...
  else if (c == 'M')
  {
    ...
    else
    {
      ...
      var contextMonth = context.month;
      if ((contextMonth + 1 > 12) || (contextMonth + 1 == 0) || (contextMonth + 1 == 00)) {
    	  context.invalid = true;
      }
  ...
  else if (c == 'd')
  {
    ...
    if((context.day == 0) || (context.day == 00) || (context.day > daysInMonth(context))) {
    	context.invalid = true;
    }
  }
  else if (c == 'E')
  ...
  global_context = context.invalid;
}
