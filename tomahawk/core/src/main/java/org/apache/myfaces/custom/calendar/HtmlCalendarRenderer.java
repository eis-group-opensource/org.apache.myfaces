 ...
 * @version $Revision: 958783 $ $Date: 2010-06-29 02:18:57 +0300 (Tue, 29 Jun 2010) $
 */
        ...
        extends HtmlRenderer
{
            ...
            DateFormatSymbols symbols) throws IOException
    {
        ...
        FacesMessage errorMessage = MessageUtils.getMessage(CONVERSION_MESSAGE_ID, new Object[]{"", null, inputCalendar.getPopupDateFormat()});
        
        //customize onchange
        if (!isDisabled(facesContext, inputCalendar) && !inputCalendar.isReadonly()) {
        	inputCalendar.setOnchange("this.value = formatValue(this.value);".concat(inputCalendar.getOnchange()!=null?inputCalendar.getOnchange():"").
        			concat("if (this.value != '') {" +
        						"var errorMessage = '" + errorMessage.getDetail() + "';" +
        				   		"if (org_apache_myfaces_dateformat_get_context()) {" +
        				   			"document.getElementById('" + inputCalendar.getClientId(facesContext) + "_error').innerHTML = errorMessage.replace(null,this.value);" +
        				   		"} else {" +
        				   			"document.getElementById('" + inputCalendar.getClientId(facesContext) + "_error').innerHTML = '';" +
        				   		"}" +
        				   		"org_apache_myfaces_dateformat_set_context(false);" +
        				   "} else {" +
        				   		"document.getElementById('" + inputCalendar.getClientId(facesContext) + "_error').innerHTML = '';}"));
        }
        //
        ...
        if (!isDisabled(facesContext, inputCalendar) && !inputCalendar.isReadonly())
        {
            ...
            //date string formatter script
            writer.write("function formatValue(value) {\n" +
            		"var re = new RegExp('[\\\\W\\\\d]', 'g');\n" +
            		"var dateFormat = " + calendarVar + ".initData.todayDateFormat ? " + calendarVar + ".initData.todayDateFormat: " + 
            			calendarVar + ".dateFormat;\n" +
            		"dateFormat = dateFormat.replace(re,'')\n" +
            		"var formater = new org_apache_myfaces_dateformat_SimpleDateFormatter(dateFormat, null, 1);\n" +
            		"if((value.search('/') == -1) && (value.length == 8)) {\n" +
            			"value = value.substring(0,2)+'/'+value.substring(2,4)+'/'+value.substring(4,8);\n"+
            		"} else {\n"+
	            		"if ((value.substring(0,3).lastIndexOf('/') == 1) && (value.length >= 6) ) {\n" +
            				"value = '0' + value;\n" +
            			"}\n" +
            			"if ((value.substring(3,10).lastIndexOf('/') == 1) && (value.length >= 6) ) {\n" +
            				"var sliced = value.slice(3,9);\n" +
            				"sliced = '0' + sliced;\n" +
            				"value = value.substring(0,3) + sliced;\n" +
            			"}\n"+
            			"if ( value.substring(6,12).length == 2 ) {\n"+ 
            				"value = value.substring(0,3)+value.substring(3,6)+'20'+value.substring(6,12);\n"+
            			"}\n"+
					"}\n"+
            		"var fromatedValue = value.replace(new RegExp('/','g'),'')\n" +
            		"var date = formater.parse(fromatedValue);\n" +
            		"if ((fromatedValue.length != dateFormat.length) || (date == null)) {\n" +
            			"org_apache_myfaces_dateformat_set_context(true);\n" +
            			"return value;" +
            		"}\n" +
            		"if (date) {\n" +
            			"return "+calendarVar+".todayDateFormatter.format(date);\n" +
            		"}\n" +
            		"return value;}");
            //
