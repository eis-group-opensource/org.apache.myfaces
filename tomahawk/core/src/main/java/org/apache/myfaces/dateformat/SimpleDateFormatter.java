...
public class SimpleDateFormatter
{
    ...
    public ParserContext prepParseOps(String dateStr) {
    	return parseOps(symbols, yearIsWeekYear, firstDayOfWeek, ops, dateStr);
    }
            ...
            String[] ops, Date date)
    {
        ...
        // E: Patched to properly handle java.sql.Date values
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        context.year = calendar.get(Calendar.YEAR);
        context.month = calendar.get(Calendar.MONTH);
        context.day = calendar.get(Calendar.DAY_OF_MONTH);
        context.dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK) - 1;
        context.hour = calendar.get(Calendar.HOUR_OF_DAY);
        context.min = calendar.get(Calendar.MINUTE);
        context.sec = calendar.get(Calendar.SECOND);
